...
scenario remains the headless evidence for the ripple and MCP path. See the
[captured console frame](../../../docs/assets/scenarios/inhouse-gc/01-stale-nda-queue.png).

## Walkthrough

1. Run the scenario to seed the fictional NDA, handbook, policy, and template library.
2. Inspect the preflight result before the fictional amendment; current NDA context is available to the assistant.
3. Register the fictional amendment to the authority anchor; all twelve NDA clauses become
   `StalePendingReverification`.
4. Inspect the post-change preflight result and console queue. No stale NDA context reaches the Copilot-like assistant;
   a lawyer must re-verify before reuse.
