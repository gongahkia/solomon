...
pub fn build(b: *std.Build) void {
    ...
    const vcs_git_libgit2_module = b.createModule(.{
        .root_source_file = b.path("src/vcs/git_libgit2.zig"),
        .target = target,
        .optimize = optimize,
    });
    const vcs_git_libgit2_debug_module = b.createModule(.{
        .root_source_file = b.path("src/vcs/git_libgit2.zig"),
        .target = target,
        .optimize = .Debug,
    });
    const vcs_git_libgit2_release_module = b.createModule(.{
        .root_source_file = b.path("src/vcs/git_libgit2.zig"),
        .target = target,
        .optimize = .ReleaseFast,
    });
    const vcs_git_libgit2_stub_module = b.createModule(.{
        .root_source_file = b.path("src/vcs/git_libgit2_stub.zig"),
        .target = target,
        .optimize = optimize,
    });
    const vcs_git_libgit2_stub_debug_module = b.createModule(.{
        .root_source_file = b.path("src/vcs/git_libgit2_stub.zig"),
        .target = target,
        .optimize = .Debug,
    });
    const vcs_git_libgit2_stub_release_module = b.createModule(.{
        .root_source_file = b.path("src/vcs/git_libgit2_stub.zig"),
        .target = target,
        .optimize = .ReleaseFast,
    });
    ...
    i18n_extract_run.addFileArg(b.path("src/cli/bench.zig"));
    ...
    const vcs_git_libgit2_tests = b.addTest(.{
        .root_module = b.createModule(.{
            .root_source_file = b.path("src/vcs/git_libgit2.zig"),
            .target = target,
            .optimize = optimize,
    ...
    const vcs_git_libgit2_test_run = b.addRunArtifact(vcs_git_libgit2_tests);
    ...
    git_branch_tests.root_module.addImport("vcs_git_libgit2", vcs_git_libgit2_module);
    ...
    i18n_extract_check_run.addFileArg(b.path("src/cli/bench.zig"));
    ...
    const zsh_async_redraw_integration = b.addSystemCommand(&.{ "bash", "test/integration/zsh_async_redraw.sh" });
    zsh_async_redraw_integration.step.dependOn(&debug_install.step);
    ...
    const context_api_integration = b.addSystemCommand(&.{ "bash", "test/integration/context_api.sh" });
    context_api_integration.step.dependOn(&debug_install.step);
    context_api_integration.step.dependOn(&debug_daemon_install.step);
    ...
    const plugin_reload_stress = b.addSystemCommand(&.{ "bash", "test/integration/plugin_reload_stress.sh" });
    plugin_reload_stress.step.dependOn(&debug_install.step);
    plugin_reload_stress.step.dependOn(&debug_daemon_install.step);
    const nix_shell_language_version = b.addSystemCommand(&.{ "bash", "test/integration/nix_shell_language_version.sh" });
    nix_shell_language_version.step.dependOn(&debug_install.step);
    nix_shell_language_version.step.dependOn(&debug_daemon_install.step);
    ...
    test_step.dependOn(&vcs_git_libgit2_test_run.step);
    if (vcs_extra) {
    ...
    test_step.dependOn(&zsh_async_redraw_integration.step);
    ...
    test_step.dependOn(&context_api_integration.step);
    ...
    test_step.dependOn(&plugin_reload_stress.step);
    test_step.dependOn(&nix_shell_language_version.step);
