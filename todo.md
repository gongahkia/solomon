...
- [x] Build a theme gallery page on shisa.sh with live previews
...
- [x] Update `docs/` whenever a user-facing flag, config field, or module changes
...
- [x] Audit log of every cloud request (hash-only by default)
...
- [x] Per-shell ergonomic shims for right_prompt where shell support is partial
...
- [x] Quarterly pack-perf review: each pack's overhead measured against baseline core
...
- [x] Triage pack issues at the pack repo level, not the core repo, once graduated
- [x] Run the pack benchmark suite weekly on the main branch
...
- [x] Fuzz harness for the deframer (libFuzzer via Zig)
...
- [x] `shisa render --explain-a11y` dumps the alt-text for the current prompt

### 21.3 `--a11y` mode
- [x] `shisa init --a11y` writes a config with colors stripped + glyphs ASCII
- [x] Built-in `a11y` theme passing AAA on ASCII
- [x] `shisa render --a11y` flag for one-shot test
...
- [x] Bell + notification preferences settable via CLI args
...
- [x] Fall through to en-US for missing translations
