[![](https://img.shields.io/badge/senko_1.0-passing-light_green)](https://github.com/gongahkia/senko/releases/tag/1.0)
[![](https://img.shields.io/badge/senko_2.0-passing-green)](https://github.com/gongahkia/senko/releases/tag/2.0)
![](https://github.com/gongahkia/senko/actions/workflows/test.yml/badge.svg)

# `せんこ`

Memorise things fast *(with [curses](https://docs.python.org/3/howto/curses.html))*. 

## Usage

```console
$ git clone https://github.com/gongahkia/senko && cd senko
$ python -m pip install .
$ senko
```

## [Quickstart](./quickstart.md)