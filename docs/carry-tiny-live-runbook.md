...
- `carry.live_armed=true`.
- `STONKS_CLI_CARRY_LIVE_ARMED=armed`.
- `carry.max_total_live_usd` manually configured between USD 50 and USD 200.
- `--cap-usd` manual confirmation matching config exactly.
- Separate secrets file at `carry.live_secrets_path`.
