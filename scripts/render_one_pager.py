...
def render(destination: Path = OUTPUT) -> Path:
    ...
    left_y = _section(canvas, "THE PROBLEM", "Search can find a useful memo but cannot establish whether its authority moved, a later view superseded it, or a lawyer contested it.", left_x, left_y, column_width)
    left_y = _section(canvas, "THE PRODUCT", "Solomon records provenance, valid and ingestion time, dependencies, verification, supersession, contestability, and metadata-only audit evidence.", left_x, left_y, column_width)
    left_y = _section(canvas, "HOW IT FITS", "An MCP host calls preflight_context before prompt assembly, then uses why, impact, or audit_pack when human review is required.", left_x, left_y, column_width)
    left_y = _section(canvas, "LIMITS", "Solomon does not decide the law, monitor every authority, or replace firm deployment, retention, confidentiality, and professional-responsibility controls.", left_x, left_y, column_width)
    ...
    right_y -= 3
    _section(canvas, "REVIEW QUESTIONS", "What is current? What changed? Which matters are affected? Who verified the position? What evidence supports reuse?", right_x, right_y, column_width)
