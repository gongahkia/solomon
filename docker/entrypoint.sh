...
for name in $(env | sed -n 's/^\(SHIBAHAMA_[^=]*\)=.*/\1/p'); do
  case "$name" in
    SHIBAHAMA_ADMIN_BOOTSTRAP_SECRET|SHIBAHAMA_ADMIN_BOOTSTRAP_TTL_SECONDS|SHIBAHAMA_ADMIN_RECOVERY_SECRET|SHIBAHAMA_API_KEY|SHIBAHAMA_BIND|SHIBAHAMA_CORS_ALLOW_CREDENTIALS|SHIBAHAMA_CORS_HEADERS|SHIBAHAMA_CORS_METHODS|SHIBAHAMA_CORS_ORIGINS|SHIBAHAMA_DATA_PATH|SHIBAHAMA_DIMENSIONS|SHIBAHAMA_ENCRYPTION_KEY|SHIBAHAMA_ENCRYPTION_KEY_ID|SHIBAHAMA_FULL_SEMANTIC_ERASURE|SHIBAHAMA_NAMESPACE|SHIBAHAMA_OIDC_AUDIENCE|SHIBAHAMA_OIDC_CA_CERT_PATH|SHIBAHAMA_OIDC_ISSUER|SHIBAHAMA_OIDC_PRINCIPAL_CLAIM|SHIBAHAMA_RATE_LIMIT_BURST|SHIBAHAMA_RATE_LIMIT_REQUESTS_PER_WINDOW|SHIBAHAMA_RATE_LIMIT_WINDOW_SECONDS|SHIBAHAMA_RBAC_ENFORCE|SHIBAHAMA_RBAC_ERASURE_MIN_ROLE|SHIBAHAMA_RBAC_PROMOTION_MIN_ROLE) ;;
...
oidc_ca_certificate="${SHIBAHAMA_OIDC_CA_CERT_PATH:-}"
oidc_principal_claim="${SHIBAHAMA_OIDC_PRINCIPAL_CLAIM:-}"
rbac_enforce="${SHIBAHAMA_RBAC_ENFORCE:-true}"
full_semantic_erasure="${SHIBAHAMA_FULL_SEMANTIC_ERASURE:-false}"
...
require_boolean "$full_semantic_erasure" "SHIBAHAMA_FULL_SEMANTIC_ERASURE"
...
if [ -n "$oidc_ca_certificate" ] && { [ ! -f "$oidc_ca_certificate" ] || [ ! -r "$oidc_ca_certificate" ]; }; then
  fail "SHIBAHAMA_OIDC_CA_CERT_PATH must reference a readable PEM file"
fi
...
[ -z "$oidc_ca_certificate" ] || set -- "$@" --oidc-ca-certificate "$oidc_ca_certificate"
[ -z "$oidc_principal_claim" ] || set -- "$@" --oidc-principal-claim "$oidc_principal_claim"
[ "$rbac_enforce" = false ] || set -- "$@" --rbac-enforce
[ "$full_semantic_erasure" = false ] || set -- "$@" --full-semantic-erasure
