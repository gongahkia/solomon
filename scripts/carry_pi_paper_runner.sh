...
: "${STONKS_CLI_CARRY_DURATION_HOURS:=720}"
...
mkdir -p "$run_dir"
printf '%s\n' "{\"event\":\"run_started\",\"run_id\":\"$run_id\",\"timestamp\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"os\":\"$(uname -s)\",\"hostname\":\"$(hostname)\"}" >> "$STONKS_CLI_CARRY_STATE_DIR/runner-events.jsonl"
