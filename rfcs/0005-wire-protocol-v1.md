...
Subscribe request adds `topics: []string`. Duplicate topics increment a per-connection reference count. The first daemon line is a snapshot event whose `data.refs` object maps each distinct topic to its reference count. Later daemon replies use NDJSON delta events, with monotonically increasing `data.sequence` values per connection:
