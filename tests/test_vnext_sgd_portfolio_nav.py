...
from stonks_cli.vnext.sgd_portfolio_nav import calculate_sgd_portfolio_nav


def test_sgd_portfolio_nav_converts_holdings_and_cash_with_complete_direct_rates():
    ...
    nav = calculate_sgd_portfolio_nav(snapshot, ledger, rates)

    assert nav.holdings_value_sgd == pytest.approx(636.0)
    assert nav.cash_value_sgd == pytest.approx(8.4)
    assert nav.net_asset_value_sgd == pytest.approx(644.4)


def test_sgd_portfolio_nav_fails_closed_for_missing_rates_or_incoherent_snapshots():
    captured_at = datetime(2026, 7, 14, 12, tzinfo=UTC)
    snapshot = PortfolioSnapshot(
        "moomoo", "100", captured_at, (PortfolioHolding("100", "us", "US.AAPL", PortfolioAssetClass.EQUITY, 2.0, "USD", 400.0),)
    )
    rates = FXReferenceRateBatch("fixture", "SGD", (FXReferenceRate("EUR", "SGD", 1.45, captured_at),))
    ledger = CashLedger("moomoo", "100", captured_at, ())

    with pytest.raises(ValueError, match="missing FX"):
        calculate_sgd_portfolio_nav(snapshot, ledger, rates)
    with pytest.raises(ValueError, match="timestamps differ"):
        calculate_sgd_portfolio_nav(snapshot, CashLedger("moomoo", "100", datetime(2026, 7, 14, 13, tzinfo=UTC), ()), rates)
