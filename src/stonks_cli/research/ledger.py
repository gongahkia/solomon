...
DEFAULT_REPLAY_FIXTURE = Path("tests/fixtures/research/paper-decisions.jsonl")
DEFAULT_LEDGER_PATH = Path("docs/decision-ledger.md")
DEFAULT_TEARSHEET_PATH = Path("docs/research-fixture-tearsheet.md")
...
def render_tearsheet(traces: list[ReplayTrace]) -> str:
    metrics = build_tearsheet(traces)
    lines = [
        "# stonks-cli Fixture Tearsheet",
        "",
        "This generated artifact is backed by `tests/fixtures/research/paper-decisions.jsonl` and is safe to publish before live data exists.",
        "",
        "## Metrics",
        "",
        "| Metric | Value |",
