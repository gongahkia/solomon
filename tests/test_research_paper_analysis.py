...
from stonks_cli.research.attribution import rank_wallets_from_fixture
from stonks_cli.research.models import ExecutionMode, NormalizedTrade, TradeSide, Venue
...
def test_paper_analysis_sizes_against_follower_bankroll_not_source_wallet():
    engine = PaperAnalysisEngine(
        rankings=_rankings(),
        config=PaperAnalysisConfig(follower_bankroll_usd=1000, max_position_fraction=0.05, max_order_notional_usd=75),
    )
    ...
    assert decision.intent.mode is ExecutionMode.PAPER
...
def test_stop_loss_sets_reentry_cooldown_and_blocks_next_trade():
    engine = PaperAnalysisEngine(
        rankings=_rankings(),
        config=PaperAnalysisConfig(follower_bankroll_usd=1000, max_position_fraction=0.10, cooldown_minutes=60),
    )
    trade = NormalizedTrade(
        venue=Venue.HYPERLIQUID,
        trade_id="source-1",
...
def test_replay_fixture_produces_realistic_tearsheet_and_ledger_controls():
    replay = replay_paper_analysis_fixture(
        ...
        rankings=_rankings(),
        config=PaperAnalysisConfig(follower_bankroll_usd=1000, max_position_fraction=0.10, max_order_notional_usd=75),
    )
    ...
    assert any(record.decision == "paper_analysis_open" for record in decisions)
    ...
    assert all(record.mode is ExecutionMode.PAPER for record in decisions)
