<!-- SPDX-License-Identifier: Apache-2.0 -->

# Scenarios

- [`01-vendor-integration/`](01-vendor-integration/): a fictional MCP host omits stale firm context before drafting.
- [`currency-loop-proof/`](currency-loop-proof/): a deterministic, headless authority-change → impact → human-review → re-verification/supersession proof.
- [`evidence-to-dependency-proof/`](evidence-to-dependency-proof/): a deterministic, headless source evidence → suggestion → curator decision → confirmed-edge currency proof.
- [`governed-dependency-assertion-proof/`](governed-dependency-assertion-proof/): a deterministic, headless explicit-assertion lifecycle proof covering evidence, review, scope, provenance, revision, and audit-pack verification.
- [`adversarial-dependency-generalization-proof/`](adversarial-dependency-generalization-proof/): a synthetic, headless adversarial parser/lifecycle safety proof with a hash-locked corpus and explicit gate limits.
- [`02-biglaw-sg/`](02-biglaw-sg/): a fictional Singapore firm seeds 30 items and stales seven banking memos after a MAS Notice 626 change.
- [`03-inhouse-gc/`](03-inhouse-gc/): a fictional SEA company stales 12 NDA clauses after a PDPA-anchor change before a Copilot-like draft.
- [`04-boutique-sg/`](04-boutique-sg/): a fictional eight-lawyer Singapore boutique seeds 50 memos and stales four privacy memos through a sparse graph.
- [`stale-house-view/`](stale-house-view/): a dependency change makes a 2023 house view stale pending re-verification.
- [`currency-report/`](currency-report/): a partner-facing period report over stale, contradictory, superseded, and retired positions.
- [`internal-supersession/`](internal-supersession/): a 2024 position supersedes a 2022 position while preserving audit recall.
- [`contradiction/`](contradiction/): two live positions cite the same authority with opposite structured conclusions.
