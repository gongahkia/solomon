...
pub fn renderOperationSummary(allocator: std.mem.Allocator, cwd_path: []const u8) !?[]u8 {
    ...
    return try formatOperationSummaryAlloc(allocator, operation);
}

pub fn formatOperationSummaryAlloc(allocator: std.mem.Allocator, operation: OperationSummary) ![]u8 {
...
test "formats jj operation summary" {
    ...
    var operation = (try parseLatestOperation(std.testing.allocator, output)).?;
    defer operation.deinit(std.testing.allocator);
    const rendered = try formatOperationSummaryAlloc(std.testing.allocator, operation);
...
test "snapshots linear jj fixture repo" {
    ...
    const wc_segment = try formatWorkingCopySummaryAlloc(allocator, wc);
    defer allocator.free(wc_segment);
...
test "snapshots conflict jj fixture repo" {
    ...
    const conflict_segment = try formatConflictSummaryAlloc(allocator, conflict);
