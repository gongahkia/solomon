...
Solomon fails closed at model egress. Context must pass Kaypoh `/pseudonymize`; the mapping remains only in
process memory and is flushed after `/reidentify`. `raw_text` mode is disabled by default and requires a
per-matter opt-in.
