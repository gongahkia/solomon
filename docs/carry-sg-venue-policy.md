# stonks-cli Carry SG Venue Policy
...
Funding/basis carry is the sole live-candidate path. Wallet analysis remains research-only and cannot feed live target selection.
...
| Wallet-ranking target selection | Blocked. |

## Enforcement

- Config defaults: `legal_policy.blocked_venue_ids` blocks `polymarket`, `kalshi`, `sportsbook`, `sportsbooks`, and `bybit`.
- Config defaults: `legal_policy.blocked_strategy_classes` blocks prediction-market, sportsbook, circumvention, and wallet live-target strategy classes.
- Carry scanner and paper commands call the legal policy before venue-specific execution.
- Live guard checks block SG-restricted venues and strategy classes before arm-flag checks can pass.

## Review Before Live

Before any live Carry path can be armed, record:
