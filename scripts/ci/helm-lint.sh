...
helm template shibahama "$CHART" "${VALUES[@]}" --set storage.destroyPolicy=preserve --namespace shibahama >"$ROOT/target/shibahama-helm-preserve.yaml"
helm template shibahama "$CHART" "${VALUES[@]}" --set storage.destroyPolicy=erase --namespace shibahama >"$ROOT/target/shibahama-helm-erase.yaml"
rg -q 'kind: PersistentVolumeClaim' "$ROOT/target/shibahama-helm.yaml"
rg -q 'helm.sh/resource-policy: keep' "$ROOT/target/shibahama-helm-preserve.yaml"
! rg -q 'helm.sh/resource-policy: keep' "$ROOT/target/shibahama-helm-erase.yaml"
