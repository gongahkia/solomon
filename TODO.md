...
### Repo & tooling
- [ ] (P0) Initialise monorepo: `core/` (Rust), `bindings/python/`, `bindings/node/`, `tideline/` (TS/React), `benchmarks/`, `examples/`, `docs/`
- [ ] (P0) Set up Cargo workspace with the core crate and a `shibahama-cli` crate
- [ ] (P0) Configure `rustfmt`, `clippy`, and deny-warnings in CI
...
- [ ] (P0) Implement stable ID generation (UUID v7 for time-orderability)
- [ ] (P0) Define `Provenance` type: source kind (user / agent / file / web / tool), source ref, ingested-by
- [ ] (P0) Define bi-temporal fields: `valid_from`, `valid_to` (nullable = open interval), `ingested_at`
- [ ] (P0) Define `CredenceTier` enum: FirmAuthoritative / VerifiedSource / ModelInferred / Unverified (generic names for OSS)
- [ ] (P0) Define `Tier` enum: Hot / Warm / Cold
...
- [ ] (P1) Implement crash-safe writes (fsync policy / WAL) and a recovery path on startup
