...
from stonks_cli.whalemirror.models import DecisionRecord


def decision_journal_path() -> Path:
    return default_state_dir() / "whalemirror_decisions.jsonl"
...
def append_decision(record: DecisionRecord, path: Path | None = None) -> None:
    ...
    try:
        ...
        log_suppressed_exception(context="whalemirror.journal.append_decision", error=e, path=use_path)
...
def read_decisions(limit: int = 100, path: Path | None = None) -> list[dict[str, object]]:
    ...
    try:
        ...
        log_suppressed_exception(context="whalemirror.journal.read_decisions", error=e, path=use_path)
        return []
    ...
    for line in lines[-max(0, limit) :]:
        try:
            ...
            log_suppressed_exception(context="whalemirror.journal.parse_line", error=e, line=line)
            continue
