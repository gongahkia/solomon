...
in an encrypted coordinated full backup, guarded restore, and upgrade rehearsal. The canonical exercised topology is
one host running `docker-compose.production.yml`; it starts serialized migration
and bootstrap jobs before the long-running API, console, and one worker.
...
Application startup applies the operation-journal migration in the selected knowledge backend. First take and inspect
a coordinated backup, then run the read-only admission check and forward migration:
...
They remain authoritative state in the mixed deployment. The current operation-store version 2 migration adds an
operations status/age index and does not rewrite historical evidence or parser baselines. An older binary rejects the
unknown newer version rather than downgrading it. Restore a verified pre-upgrade checkpoint into fresh targets if
rollback is required; database downgrade is not automatic.

## Backup and restore boundary

`solomon deployment backup`, `backup-inspect`, `restore-plan`, and `restore` are the supported server operational
surface. A plan is dry-run and deterministic; `restore --apply` verifies the unchanged fingerprint and refuses an
existing local root or non-empty PostgreSQL target. PostgreSQL logical restore uses one PostgreSQL transaction, but it
is not atomic with activation of the local state volume. A crash between those component actions needs operator
recovery from an isolated target, not automatic compensation. Full procedure and failure handling are documented in
[Production Backup, Restore, And Upgrade Operations](./operations/production-backup-restore.md).
