...
from stonks_cli.whalemirror.carry_audit import CarryReconciliationReport
from stonks_cli.whalemirror.hyperliquid import HyperliquidCarryInput, HyperliquidOrderClient, HyperliquidOrderIntent
from stonks_cli.whalemirror.models import CarryDecision, MirrorMode, Venue
...
def evaluate_carry_live_preflight(
    *,
    ...
    blockers: list[str] = []
    env_value = use_env.get(cfg.carrymirror.live_armed_env, "").strip().lower()
    cap = cfg.carrymirror.max_total_live_usd
    secrets_path = Path(evidence.secrets_path or cfg.carrymirror.live_secrets_path).expanduser()
    if not cfg.carrymirror.live_armed:
        blockers.append("carry_live_config_not_armed")
    if env_value not in TRUTHY_CARRY_ARM_VALUES:
        blockers.append(f"carry_live_env_not_armed:{cfg.carrymirror.live_armed_env}")
    ...
    if cap < cfg.carrymirror.tiny_live_min_usd or cap > cfg.carrymirror.tiny_live_max_usd:
    ...
    return CarryLivePreflight(ok=not blockers, blockers=blockers, env_var=cfg.carrymirror.live_armed_env, cap_usd=cap)
