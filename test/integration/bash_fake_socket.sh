...
loop do
  ...
  begin
    ...
    # A prompt client may enforce its 5 ms budget before this test process is
    # scheduled. Keep serving retries; the production timeout is intentional.
  ensure
...
if ((BASH_VERSINFO[0] >= 4)); then
  for _ in {1..20}; do
    ...
    grep -F 'fake-bash> ' "$out" >/dev/null && break
    sleep 0.01
  done
else
  for _ in {1..20}; do
...
if ! grep -F 'fake-bash> ' "$out" >/dev/null; then
  printf 'bash fake socket: prompt did not complete within retry budget\n' >&2
  cat "$out" >&2
  exit 1
fi
...
if ((BASH_VERSINFO[0] >= 4)); then
  ...
  bash --noprofile --norc -c 'source init/shisa.bash; trap - DEBUG; SHISA_COMMAND_STARTED=0; SHISA_COMMAND_START_US=; TEST_NOW_US=100000000; shisa_epoch_us() { printf "%s" "$TEST_NOW_US"; }; shisa_debug_trap "sleep 1"; TEST_NOW_US=101234000; shisa_precmd 7; [[ ${SHISA_LAST_EXIT} == 7 ]]; [[ ${SHISA_LAST_DURATION_MS} == 1234 ]]'
